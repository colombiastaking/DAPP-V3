...
async function main() {
  ...
  console.log("🚀 EXECUTING DAO POOL DISTRIBUTION");
  console.log("═".repeat(70));
  ...
  const recipients = recipientsData.daoRecipients;
  const totalAmount = recipients.reduce((s, r) => s + r.dailyDao, 0);
  ...
  for (const recipient of recipients) {
    const amountBigInt = BigInt(Math.floor(recipient.dailyDao * 1e18));
    ...
    try {
      ...
      txHashes.push({ hash: txHash.toString(), recipient: recipient.address, amount: recipient.dailyDao });
      successCount++;
      console.log(`✅ [${successCount}/${recipients.length}] ${recipient.address.slice(0,20)}... → ${recipient.dailyDao.toFixed(4)} COLS (${txHash.toString().slice(0,10)}...)`);
    } catch (e) {
      failCount++;
  ...
  // Save results
  fs.writeFileSync('/tmp/dao_distribution_results.json', JSON.stringify({
  ...
  console.log(`\n✅ Results saved to /tmp/dao_distribution_results.json`);
}
